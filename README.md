...
2. **실행**:
    ...
    *   **진입점 원칙**: 루트의 `open-*.bat` 파일은 `launchers/` 안의 실제 구현을 호출하는 사용자 진입점입니다. 동작 변경은 먼저 `launchers/`를 기준으로 검토하십시오.
...
4. **배포/원격 접근**: 기본값은 로컬 전용입니다. LAN 또는 인터넷 원격 접근이 필요하면 먼저 [docs/15-deployment-and-access.md](docs/15-deployment-and-access.md)의 체크리스트를 확인하십시오.
...
*   **docs/15-deployment-and-access.md**: 로컬/LAN/원격 접근 기준과 배포 체크리스트
...
### 자동 테스트 (CI 범위)
GitHub Actions(Ubuntu, Python 3.11)에서 푸시/PR 시 자동으로 실행되는 테스트입니다.
*   **실행**: `python -m pytest tests/ -q`
*   **특징**: CI는 `requirements.txt`와 `pytest`를 설치하고, 임시 데이터/DB/워크다이어리 경로와 테스트 환경을 준비한 뒤 운영 데이터와 격리된 상태에서 수행됩니다.
...
<p>Push retry at 2026-04-13 17:59</p>

*   `scripts/` package movement의 안전한 이동 범위는 대부분 분리되었고, 남은 server/DB/Telegram core 영역은 별도 narrow issue가 필요한 고위험 영역입니다. 세부 경계는 [docs/scripts-package-movement-audit.md](docs/scripts-package-movement-audit.md)를 참고하십시오.
...
*   로컬 외 접근 경로와 배포 체크리스트는 [docs/15-deployment-and-access.md](docs/15-deployment-and-access.md)에 정리했습니다.

## 저장소 구조 요약

*   `data/`: 데이터베이스, 런타임 상태 및 파일 큐 (`data/queue/`)
*   `docs/`: 운영/설계 문서. `scripts/` package movement 기록은 [docs/scripts-package-movement-log.md](docs/scripts-package-movement-log.md), 현재 경계와 고위험 지연 영역은 [docs/scripts-package-movement-audit.md](docs/scripts-package-movement-audit.md)를 참고하십시오.
*   `launchers/`: Windows용 보조 launcher들. 일부는 WSL/bash 환경을 전제로 함
*   `public/`: 웹 프론트엔드 자산
*   `scripts/`: 주요 운영 스크립트, 통합 세션 래퍼, 보조 CLI. 역할별 subpackage로 점진 분리 중이며, 기존 flat command/import path는 호환 wrapper로 유지합니다.
    *   `scripts/checks/`: CI/static/smoke 검증 구현. 기존 command path는 wrapper로 유지합니다.
    *   `scripts/cli/`: 사용자-facing CLI와 standalone command entrypoint의 canonical 위치입니다. DB integrity, contract validation, session/inbox CLI, Telegram CLI, Telegram attachment/backfill helper가 이 영역에 포함됩니다.
    *   `scripts/queue_runtime/`: 파일 큐, queue worker, report export, verdict import 등 파일 기반 판정 파이프라인 runtime helper의 canonical 위치입니다. `scripts/queue/` 이름은 Python stdlib `queue` shadow 위험 때문에 사용하지 않습니다.
    *   `scripts/integrations/`: Telegram progress, session lock, service/helper 등 외부 서비스 integration helper의 canonical 위치입니다.
    *   `scripts/db/`: DB helper 일부를 점진적으로 이동 중입니다. 기존 `scripts/db_*.py` 경로는 호환 wrapper로 유지합니다.
    *   `scripts/services/`: server handler와 CLI가 공유하는 service helper 일부를 점진적으로 이동 중입니다.
    *   `scripts/server_handlers/`: HTTP route handler 분리 영역입니다.
    *   Telegram runtime service와 Telegram DB helper 중 `scripts/telegram_service.py`, `scripts/telegram_db.py`는 public constants와 test patch/mutation 호환성 때문에 아직 flat path에 남겨둡니다. DB/session core인 `scripts/db.py`, `scripts/db_base.py`, `scripts/db_sessions.py`도 별도 narrow issue 전까지 유지합니다. 세부 기준은 [docs/scripts-telegram-migration-audit.md](docs/scripts-telegram-migration-audit.md)와 [docs/scripts-package-movement-audit.md](docs/scripts-package-movement-audit.md)를 참고하십시오.
*   `tests/`: 테스트 스크립트