...
*   `docs/`: 운영/설계 문서. `scripts/` package movement 기록은 [docs/scripts-package-movement-log.md](docs/scripts-package-movement-log.md)를 참고하십시오.
...
*   `scripts/`: 주요 운영 스크립트, 통합 세션 래퍼, 보조 CLI. 역할별 subpackage로 점진 분리 중이며, 기존 flat command/import path는 호환 wrapper로 유지합니다.
    ...
    *   Telegram runtime service와 Telegram DB helper 중 `scripts/telegram_service.py`, `scripts/telegram_db.py`는 public constants와 test patch/mutation 호환성 때문에 아직 flat path에 남겨둡니다. 세부 기준은 [docs/scripts-telegram-migration-audit.md](docs/scripts-telegram-migration-audit.md)를 참고하십시오.
*   `tests/`: 테스트 스크립트