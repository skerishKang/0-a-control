...
def _has_fk_on_column(conn: sqlite3.Connection, table: str, column: str, ref_table: str) -> bool:
...
def apply_source_records_session_fk(conn: sqlite3.Connection) -> None:
    ...
    """
    if not _table_exists(conn, "source_records") or not _table_exists(conn, "sessions"):
        return
    ...
    # 1. Drop FTS triggers (will be recreated by rebuild_fts / FTS_SCHEMA)
    ...
    # 4. Copy data
    conn.execute(
        f"INSERT INTO source_records ({col_sql}) SELECT {col_sql} FROM source_records_old"
    )
