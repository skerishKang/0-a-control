...
# Large SQL string constants that don't depend on any other module.
...
CREATE TRIGGER IF NOT EXISTS sessions_ai AFTER INSERT ON sessions BEGIN
   INSERT INTO sessions_fts(id, title, summary_md) VALUES (NEW.id, COALESCE(NEW.title, ''), COALESCE(NEW.summary_md, ''));
END;
...
CREATE TRIGGER IF NOT EXISTS source_records_ai AFTER INSERT ON source_records BEGIN
   INSERT INTO source_records_fts(id, source_name, content) VALUES (NEW.id, COALESCE(NEW.source_name, ''), COALESCE(NEW.content, ''));
END;
...
CREATE TRIGGER IF NOT EXISTS brief_records_ai AFTER INSERT ON brief_records BEGIN
   INSERT INTO brief_records_fts(id, title, content_md) VALUES (NEW.id, COALESCE(NEW.title, ''), COALESCE(NEW.content_md, ''));
END;
...
CREATE TRIGGER IF NOT EXISTS decision_records_ai AFTER INSERT ON decision_records BEGIN
   INSERT INTO decision_records_fts(id, title, reason, impact_summary) VALUES (NEW.id, COALESCE(NEW.title, ''), COALESCE(NEW.reason, ''), COALESCE(NEW.impact_summary, ''));
END;
...
CREATE TRIGGER IF NOT EXISTS external_inbox_ai AFTER INSERT ON external_inbox BEGIN
   ...
   VALUES (CAST(NEW.id AS TEXT), COALESCE(NEW.source_name, ''), COALESCE(NEW.author, ''), COALESCE(NEW.title, ''), COALESCE(NEW.raw_content, ''), COALESCE(NEW.attachment_path, ''));
END;
...
def rebuild_fts(conn: sqlite3.Connection) -> None:
    """Rebuild all full-text search indexes from current table data."""
    conn.execute("DELETE FROM sessions_fts")
    conn.execute(
        "INSERT INTO sessions_fts(id, title, summary_md) "
        "SELECT id, COALESCE(title, ''), COALESCE(summary_md, '') FROM sessions"
    )

    conn.execute("DELETE FROM source_records_fts")
    conn.execute(
        "INSERT INTO source_records_fts(id, source_name, content) "
        "SELECT id, COALESCE(source_name, ''), COALESCE(content, '') FROM source_records"
    )

    conn.execute("DELETE FROM brief_records_fts")
    conn.execute(
        "INSERT INTO brief_records_fts(id, title, content_md) "
        "SELECT id, COALESCE(title, ''), COALESCE(content_md, '') FROM brief_records"
    )

    conn.execute("DELETE FROM decision_records_fts")
    conn.execute(
        "INSERT INTO decision_records_fts(id, title, reason, impact_summary) "
        "SELECT id, COALESCE(title, ''), COALESCE(reason, ''), COALESCE(impact_summary, '') FROM decision_records"
    )

    conn.execute("DELETE FROM external_inbox_fts")
    conn.execute(
        "INSERT INTO external_inbox_fts(id, source_name, author, title, raw_content, attachment_path) "
        "SELECT CAST(id AS TEXT), COALESCE(source_name, ''), COALESCE(author, ''), "
        "COALESCE(title, ''), COALESCE(raw_content, ''), COALESCE(attachment_path, '') FROM external_inbox"
    )


def backfill_event_log(conn: sqlite3.Connection) -> None:
    """Populate event_log from existing sessions and decision_records."""
    rows = conn.execute(
        "SELECT id, title, started_at, ended_at, status, project_key, agent_name FROM sessions"
    ).fetchall()
    for row in rows:
        metadata = {
    ...
    rows = conn.execute(
        "SELECT id, decision_type, title, reason, created_at, related_quest_id, related_plan_item_id, related_session_id FROM decision_records"
    ).fetchall()
    for row in rows:
        conn.execute(
            """
