...
def apply_schema_migrations(conn: sqlite3.Connection) -> None:
    ...
    if not schema_migration_applied(conn, DECISION_RECORDS_REFERENCE_FK_VERSION):
        from scripts.db_fk_migrations import apply_decision_records_reference_fks
    ...
    if not schema_migration_applied(conn, DECISION_RECORDS_SESSION_FK_VERSION):
        from scripts.db_fk_migrations import apply_decision_records_session_fk
    ...
    if not schema_migration_applied(conn, PLAN_ITEMS_SESSION_FK_VERSION):
        from scripts.db_fk_migrations import apply_plan_items_session_fk

        apply_plan_items_session_fk(conn)
        record_schema_migration(
            conn,
