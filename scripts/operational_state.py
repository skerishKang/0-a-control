...
def classify_issue(issue: dict[str, Any], now: datetime | None = None) -> Classification:
    ...
    if state == "closed":
        return Classification(
            ...
            next_action="No action required unless it should be revisited.",
            priority="P3",
        )
    ...
    if _has_any(values, ("review", "needs review", "검토")):
        return Classification(
            ...
            next_action="Review the scope and decide whether implementation or validation is next.",
...
def classify_pull_request(pr: dict[str, Any], now: datetime | None = None) -> Classification:
    ...
    if state == "closed":
        if pr.get("merged_at"):
            return Classification(
                status=STATUS_DONE,
                reason="Pull request was integrated.",
    ...
    if _has_any(values, ("blocked", "hold", "waiting")):
        return Classification(
            ...
            next_action="Identify the blocker before review.",
            guards=(GUARD_REVIEW_REQUIRED,),
            priority="P1",
        )
    ...
    if _has_any(values, ("docs", "documentation")):
        return Classification(
            ...
            next_action="Review documentation scope and readiness.",
            guards=(GUARD_REVIEW_REQUIRED,),
            priority="P2",
        )
    ...
    if _is_stale(pr, now):
        return Classification(
            ...
            next_action="Review current relevance and decide whether to continue.",
            guards=(GUARD_REVIEW_REQUIRED,),
            priority="P2",
        )
