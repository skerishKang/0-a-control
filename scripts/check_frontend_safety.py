...
This checker blocks high-risk script URL patterns immediately, prevents the
legacy inline event-handler count from increasing beyond an explicit baseline,
and requires local CSS/JS entry assets loaded from HTML to include version
queries. Raw innerHTML assignments are reported as warnings only for now.
"""
...
DEFAULT_INLINE_EVENT_BASELINE = int(os.getenv("CONTROL_TOWER_FRONTEND_INLINE_EVENT_BASELINE", "31"))
