...
VALID_KEYS = {"type", "source_text", "korean_translation", "study_note"}
    ...
    for line in content.split("\n"):
        if line.startswith("# Page "):
            if current_block and current_key:
                current_block[current_key] = current_block[current_key].rstrip("\n")
                current_key = None
            if current_block:
...
for md_name, html_name in files_to_process:
    ...
    print(f"saved: {html_name} rebuilt ({len(pages)} pages, {total_blocks} blocks)")
    for p in pages:
        for b in p["blocks"]:
            src_len = len(b["source_text"])
            trn_len = len(b["korean_translation"])
            note_len = len(b["study_note"])
            if src_len == 0 or trn_len == 0 or note_len == 0:
                print(f"  WARNING: Block {b['num']} ({b['type']}) has empty field(s): src={src_len} trn={trn_len} note={note_len}")
