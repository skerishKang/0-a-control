...
class SourceRecordsFKMigrationTests(unittest.TestCase):
    ...
    def test_foreign_key_list_has_session_id(self) -> None:
        ...
        try:
            ...
            # on_delete column index is 6
            self.assertEqual(session_fk[0][6], "SET NULL")
        finally:
            conn.close()
    ...
    def test_fts_trigger_survives_migration(self) -> None:
        ...
        try:
            conn.execute(
                """INSERT INTO sessions
                   (id, agent_name, source_type, started_at, status)
                   VALUES ('sess-fts', 'test-agent', 'test', '2026-01-01', 'active')"""
            )
            conn.execute(
                """INSERT INTO source_records
                   (id, source_type, source_name, session_id, content, created_at)
                   VALUES ('sr-fts', 'test', 'tester', 'sess-fts', 'searchable content', '2026-01-01')"""
            )
            conn.commit()

            fts_row = conn.execute(
                "SELECT id FROM source_records_fts WHERE id = 'sr-fts'"
            ).fetchone()
            self.assertIsNotNone(fts_row)
            self.assertEqual(fts_row["id"], "sr-fts")
        finally:
            conn.close()
